    ...
    <main className="mx-auto max-w-6xl px-4 py-10">
      <section className="relative mt-6 overflow-hidden rounded-3xl border bg-white p-8 md:p-12">
        ...
        <div className="relative">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href={`/en/contact`} className="rounded-xl border bg-black px-5 py-3 text-sm font-bold text-white">
              {t.hero.cta}
            </Link>
            <Link href={`/en/projects`} className="rounded-xl border px-5 py-3 text-sm font-bold">
              See projects
            </Link>
          </div>

          <div className="mt-8 grid gap-3 md:grid-cols-3">
            <Stat title="RFID + PLC + HMI" desc="Full automation integration." />
            <Stat title="Weighing & sorting" desc="Control and data logging." />
            <Stat title="Custom-built" desc="Designed for your farm." />
          </div>
        </div>
      </section>
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href={`/en/services`} className="text-sm font-semibold text-black/70 hover:text-black">
            View all →
          </Link>
        </div>
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4">
          ...
          <Link href={`/en/projects`} className="text-sm font-semibold text-black/70 hover:text-black">
            View all →
          </Link>
        </div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          {projects.slice(0, 3).map((p) => (
            <Link key={p.slug} href={`/en/projects/${p.slug}`} className="group rounded-2xl border bg-white p-5 transition hover:shadow-md">
              <div className="aspect-[16/10] overflow-hidden rounded-xl border bg-black/5">
                {p.coverImage ? (
                  <img src={p.coverImage} alt={p.title[locale]} className="h-full w-full object-cover transition group-hover:scale-[1.02]" />
                ) : null}
              </div>
      ...
      <section className="mt-12 rounded-3xl border bg-black p-8 text-white md:p-10">
        <h2 className="text-2xl font-extrabold md:text-3xl">Want to automate your farm?</h2>
        <p className="mt-3 max-w-2xl text-white/80">
          Tell us your case and we’ll propose a custom solution (weighing, sorting, feeding, integration).
        </p>
        <div className="mt-6">
          <Link href={`/en/contact`} className="inline-block rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-black">
            Contact
          </Link>
        </div>
      </section>
